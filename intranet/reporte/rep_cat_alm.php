...
u.sigla as 'UM',
case p.estado when 'A' then 'Activo' when 'I' then 'Inactivo' end as 'estado'
FROM
subcategoria s
INNER JOIN producto p
ON s.idsubcategoria = p.idsubcategoria
INNER JOIN unidad_medida u
ON p.idunidad_medida = u.idunidad_medida");
$header = array('Nº', 'producto', 'precio','subcategoria', 'UM','estado');
$i = 0;
while($row = mysqli_fetch_array($rs)){
	$data[$i] = array(($i + 1),
				utf8_decode($row['producto']),
				utf8_decode($row['precio']),
				utf8_decode($row['subcategoria']),
				utf8_decode($row['UM']),
